...
export const AuthProvider = (props) => {
  ...
  const updateUserPassword = async (body) => {
    try {
      const response = await callAPI("/password", "POST", {
        ...body,
        id: user.id,
      });
      if (!response.user) {
  ...
  const updateCart = async (order_product) => {
    ...
    if (cart) {
      ...
      const op = order_products.find(
        (op) => op.variant.id == order_product.variant.id
      );
      ...
      if (op) {
        ...
        op.total_price =
          parseFloat(op.total_price) + parseFloat(order_product.total_price);
        const updated_op = await updateOrderProduct(
          op.id,
          op.quantity,
          op.total_price
        );
      ...
      try {
        const body = {
          total_price:
            ...
            parseFloat(deleted_order_product.total_price),
        };
        const data = await callAPI(`/carts/${cart.id}`, "PUT", body);
        setCart(data);
      } catch (err) {
