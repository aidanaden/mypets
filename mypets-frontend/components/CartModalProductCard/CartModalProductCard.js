...
function CartModalProductCard({ order_products, onClose }) {
  ...
  const addQuantities = (variant) => {
    ...
    if (
      existing_order_product.variant.discounted_price &&
      existing_order_product.variant.discounted_price > 0
    ) {
      orderProductPrice = existing_order_product.variant.discounted_price;
    } else {
      orderProductPrice = existing_order_product.variant.price;
    }

    const order_product = {
      variant: existing_order_product.variant,
      quantity: 1,
      total_price: parseFloat(orderProductPrice),
    };
  ...
  const minusQuantities = (variant) => {
    ...
    if (tempQuantities[variant] > 1) {
      ...
      const order_product = {
        variant: existing_order_product.variant,
        quantity: -1,
        total_price: parseFloat(orderProductPrice) * -1.0,
      };
  ...
  useEffect(() => {
    if (order_products) {
      ...
      const categorized_products = order_products[0].variant
        .variant_type_is_float
  ...
  return (
    <Box mb={{ base: 6, lg: 8 }} w="100%" minW={{ lg: "480px" }}>
      {order_products ? (
        <>
          <HStack mb={6}>
            <NextLink
              ...
              <a onClick={onClose}>
                ...
                >
                  ...
                  >
                    ...
                    />
                    <Box ml={4}>
                      <Text fontWeight="bold" fontSize={{ base: "md" }}>
                        {order_products[0].variant.product.name}
                      </Text>
                      <MerchantBadge
                        merchantName={
                          order_products[0].variant.product.merchant.name
                        }
                      />
                    </Box>
                  </Flex>
              ...
              {variants.map((variant, i) => (
                <Tr key={i}>
                  <Th textAlign="center" fontSize="sm">
                    {variant}
                    {productsCategorized[variant][0].variant
                      .variant_type_is_float &&
                      productsCategorized[
                        variant
                      ][0].variant.product.unit.toLowerCase()}
                  </Th>
                  <Th textAlign="center" fontSize="sm">
                    SG$
                    {productsCategorized[variant][0].variant.discounted_price
                      ? productsCategorized[
                          variant
                        ][0].variant.discounted_price.toFixed(2)
                      : productsCategorized[variant][0].variant.price.toFixed(
                          2
                        )}
                  </Th>
                  <Th textAlign="center" fontSize="sm">
