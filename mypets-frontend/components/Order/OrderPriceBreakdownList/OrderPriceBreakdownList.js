...
function OrderPriceBreakdownList({
  ...
  return (
    <>
        ...
        <Tbody>
          {productNames.map((productName, i) => (
            <Tr key={i}>
              ...
              <Td py={2} textAlign="center" fontWeight="bold">
                {productTotalQuantity(groupedOrderProducts[productName])}
              </Td>
              <Td py={2} textAlign="right" fontWeight="bold">
                {productTotalPrice(groupedOrderProducts[productName]).toFixed(
                  2
                )}
              </Td>
            </Tr>
      ...
      <Box mt={4}>
        <BreakdownRow text="Subtotal" value={order.total_price.toFixed(2)} />
        ...
        <BreakdownRow text="Delivery" value={order.shipping_fee.toFixed(2)} />
        {order.tax_fee > 0 && (
          <BreakdownRow text="Service fee" value={order.tax_fee.toFixed(2)} />
        )}
        <BreakdownRow
